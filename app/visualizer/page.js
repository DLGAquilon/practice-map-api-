  ...
  const animateShortestPath = (nodesInShortestPathOrder) => {
    for (let i = 0; i < nodesInShortestPathOrder.length; i++) {
      setTimeout(() => {
        ...
        document.getElementById(`node-${node.row}-${node.col}`).className =
          "w-[25px] h-[25px] border-[0.5px] border-brand-ochre/10 transition-all duration-500 bg-node-path z-10 scale-125";
      }, 50 * i);
    }
  };
  ...
  return (
    <button
      ...
      className={`px-3 py-1.5 text-[10px] font-bold uppercase tracking-tighter rounded transition-all ${
        active ? `${color} text-white ring-2 ring-white/20` : "text-stone-400 hover:text-stone-200"
      }`}
    >
      {label}
    </button>
